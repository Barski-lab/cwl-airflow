#!/usr/bin/env python
import os
import sys
import argparse
from cwl_airflow.utils.mute import suppress_stdout, restore_stdout
suppress_stdout()  # Suppress output generated by all imported modules
from airflow import configuration
from airflow.bin.cli import scheduler
from cwl_airflow.utils.func import export_job_file, update_args
from cwl_airflow.utils.utils import update_config, copy_cwl_dag, create_folders, get_workflow_output, normalize_args
restore_stdout()  # Restore output


def arg_parser():
    parent_parser = argparse.ArgumentParser(add_help=False)
    general_parser = argparse.ArgumentParser(description='cwl-airflow')

    subparsers = general_parser.add_subparsers()
    subparsers.required = True

    init_parser = subparsers.add_parser('init', help="Init cwl-airflow", parents=[parent_parser])
    init_parser.set_defaults(func=run_init)

    run_parser = subparsers.add_parser('run', help="Run workflow", parents=[parent_parser])
    run_parser.set_defaults(func=run_job)
    run_parser.add_argument("-o", "--output", dest='output_folder', type=str,
                            help="Output folder, default current directory", default=os.path.abspath('.'))
    run_parser.add_argument("-t", "--tmp", dest='tmp_folder', type=str, help="Temporary folder")
    run_parser.add_argument("workflow", type=str)
    run_parser.add_argument("job", type=str)

    return general_parser


def run_init(**kwargs):
    update_config(configuration)
    with open(configuration.AIRFLOW_CONFIG, 'w') as cfg_file:
        configuration.conf.write(cfg_file)
    copy_cwl_dag(configuration)
    create_folders(configuration)


def run_job(args):
    suppress_stdout()
    export_job_file(args)
    update_args(args)
    scheduler(args)
    restore_stdout()
    print(get_workflow_output(args.dag_id))


def main(argsl=None):
    if argsl is None:
        argsl = sys.argv[1:]
    args, _ = arg_parser().parse_known_args(argsl)
    args = normalize_args(args, skip_list=["func"])
    args.func(args)


if __name__ == "__main__":
    sys.exit(main(sys.argv[1:]))
