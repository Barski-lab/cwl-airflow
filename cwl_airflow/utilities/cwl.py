...
from cwl_airflow.utilities.helpers import (get_md5_sum, get_dir, load_yaml,
                                           dump_json, get_absolute_path,
                                           get_rootname,
                                           remove_field_from_dict,
                                           get_uncompressed, get_compressed,
                                           get_files, get_dir_size)
...
def get_workflow_execution_stats(context):
    ...
    try:
        statistics = {
            "version":
            "1.0",  # upgrade the version when we decide to change the logic of how we collect statistics
            "total": {
                "tmp_folder_size":
                get_dir_size(job_data["tmp_folder"]),
                "outputs_folder_size":
                get_dir_size(job_data["outputs_folder"]),
                "start_date":
                dag_run.start_date.isoformat(),
                "end_date":
                round((dag_run.end_date - dag_run.start_date).total_seconds(),
                      3)
            },
            "steps": {}
        }
        for ti in context["dag_run"].get_task_instances():
            step_tmp_folder, _, _, _ = get_temp_folders(task_id=ti.task_id,
                                                        job_data=job_data)
            statistics["steps"][ti.task_id] = {
                "tmp_folder_size":
                get_dir_size(step_tmp_folder),
                "start_date":
                round((ti.start_date - dag_run.start_date).total_seconds(), 3),
                "end_date":
                round((ti.end_date - dag_run.start_date).total_seconds(), 3)
            }
...
def clean_dag_run_db(dag_run, session=None):
    ...
    for ti in dag_run.get_task_instances():
        ...
        session.query(TaskInstance).filter(
            ...
            TaskInstance.execution_date == dag_run.execution_date).delete(
...
def conf_get(section, key, default, as_boolean=None):
    """
    ...
    try:
        ...
        logging.disable(
            logging.NOTSET)  # is guaranteed to be executed before any return
...
def get_default_cwl_args(preset_cwl_args=None):
    ...
    required_cwl_args.update({
        ...
        "keep_tmp_data":
        conf_get(  # prevents from cleaning dag_run temp data and XCom's in both success or failure scenarios
            "cwl",
            "keep_tmp_data",
        ...
        "use_container":
        conf_get(  # execute jobs in docker containers
            "cwl",
            "use_container",
        ...
        "no_match_user":
        conf_get(  # disables passing the current uid to "docker run --user"
            "cwl",
            "no_match_user",
        ...
        "skip_schemas":
        conf_get(  # it looks like this doesn't influence anything in the latest cwltool
            "cwl",
            "skip_schemas",
...
def relocate_outputs(
    ...
    relocated_job_data['workflow_def'] = workflow_tool
    relocated_job_data['workflow_params'] = params
...
def get_containers(job_data, task_id):
    ...
    step_tmp_folder, _, _, _ = get_temp_folders(task_id=task_id,
                                                job_data=job_data)
...
def need_to_run(workflow_data, job_data, task_id):
    ...
    if "when" in selected_step:
        return do_eval(
            ...
            resources={},
            timeout=
            120  # harcoded to 120, because default 30 sec might not be enough when running from Docker
        )
    else:
        return True
    ...
    if isinstance(data, MutableMapping):
        ...
        for item in data:
            if isinstance(item, str):
                ...
                if target_id is not None:
                    if item["id"] == target_id or get_short_id(
                            item["id"]) == target_id:
...
def get_short_id(long_id, only_step_name=None, only_id=None):
    ...
    if len(part.split(
            "/")) > 2:  # if "id" has weird stuff between step name and id
        part = "/".join([part.split("/")[0], part.split("/")[-1]])

    if len(part.split(
            "/")) == 1:  # if fragment is only one word (based on "/")
...
def slow_cwl_load(workflow, cwl_args=None, only_tool=None):
    ...
    def __load(location):
        return load_tool(
            location,
            setup_loadingContext(LoadingContext(default_cwl_args),
                                 RuntimeContext(default_cwl_args),
                                 argparse.Namespace(**default_cwl_args)))
    ...
    try:
        with NamedTemporaryFile(
                mode="w"
        ) as temp_stream:  # guarantees that temp file will be removed
            json.dump(get_uncompressed(workflow, parse_as_yaml=True),
                      temp_stream)
