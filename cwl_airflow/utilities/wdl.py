...
WDL_OUTPUTS_FOLDER = os.path.join(AIRFLOW_HOME, "wdl_outputs_folder")
WDL_INPUTS_FOLDER = os.path.join(AIRFLOW_HOME, "wdl_inputs_folder")
WDL_PICKLE_FOLDER = os.path.join(AIRFLOW_HOME, "wdl_pickle_folder")
WDL_USE_CONTAINER = True
WDL_KEEP_TMP_DATA = False
WDL_NO_MATCH_USER = False
WDL_SKIP_SCHEMAS = True
WDL_STRICT = False
WDL_QUIET = False
# better not to change without need, however if we "copy", this will be always False, and that's good for disk usage statistics
WDL_RM_TMPDIR = True
# we want to "copy" instead of "move" to calculate disk usage from the dag callback
WDL_MOVE_OUTPUTS = "copy"
WDL_ENABLE_DEV = True      # better not to change without need


DAG_TEMPLATE = """#!/usr/bin/env python3
from wdl_airflow.extensions.wdldag import WDLDAG
dag = WDLDAG(
    workflow="{0}",
    dag_id="{1}"
)
...
def get_workflow_execution_stats(context):
    ...
    try:
        statistics = {
            # upgrade the version when we decide to change the logic of how we collect statistics
            "version": "1.0",
            "total": {
        ...
        logging.info(
            f"Failed to collect statistics for dag_id: {dag_run.dag_id}, run_id: {dag_run.run_id}, due to\n {err}")
    return statistics
...
def overwrite_deprecated_dag(
    ...
    all our old DAGs always have absolute path to the WDL file). Function doesn't backup
    or update the original WDL file.
    ...
    with open(dag_location, "r+") as io_stream:                # open for both reading and writing
        ...
        # do nothing if it wasn't old-style DAG
        if not re.search("dag\\.create\\(\\)", dag_content):
            return
        ...
        compressed_workflow_content = get_compressed(
            # no "run" embedding or convertion to Workflow. If DAG worked, wdl should be ok too
            fast_wdl_load(workflow_location)
        )

        # copy old DAG to the folder with deprecated DAGs, add ".airflowignore"
        if deprecated_dags_folder is not None:
            # try to create "deprecated_dags_folder" if it doesn't exist
            get_dir(deprecated_dags_folder)
            shutil.copy(dag_location, deprecated_dags_folder)  # copy DAG file
            ignore = os.path.join(
                deprecated_dags_folder,
                ".airflowignore"
            )
            # add deprecated DAG to ".airflowignore"
            with open(ignore, "a") as output_stream:
                output_stream.write(
                    os.path.basename(dag_location) + "\n"
                )

        # rewind "dag_location" file to the beginning
        io_stream.seek(0)
        io_stream.write(
            DAG_TEMPLATE.format(
...
def remove_outdated_dags(wdl_id, dags_folder):
    """
    ...
    rule "wdldid-commitsha", we check if there are any files that have target wdl_id in the
    rootname (aka in the dag_id). For all collected DAGs (based on wdl_id) we save modified
    timestamp and location, then sort them by timestamp excluding the newest one, thus
    forming a list of outdated DAGs for the same wdl_id (the same workflow). Then we iterate
    ...
    logging.info(
        f"Searching for dags based on wdl_id: {wdl_id} in order to remove the old ones")
    dags = {}
    for location in list_py_file_paths(dags_folder, include_examples=False):
        dag_id = get_rootname(location)
        if wdl_id not in dag_id:
            continue
        dags[dag_id] = {
            "location": location,
    ...
    for dag_id, dag_metadata in sorted(dags.items(), key=lambda i: i[1]["modified"])[:-1]:
        ...
        if len(DagRun.find(dag_id=dag_id, state=State.RUNNING)) == 0:
            ...
            for f in [
                ...
                os.path.splitext(dag_metadata["location"])[0]+".wdl"
            ]:
                try:
...
def stop_dag_run_tasks(dag_run, timeout):
    ...
    logging.info(
        f"Stopping running tasks for dag_id: {dag_run.dag_id}, run_id: {dag_run.run_id}")
    for ti in dag_run.get_task_instances():
        logging.info(
            f"Task: {ti.task_id}, execution_date: {ti.execution_date}, pid: {ti.pid}, state: {ti.state}")
        if ti.state == State.RUNNING:
            ...
            if process:
                ...
                try:
                    ...
                    logging.info(
                        " - done waiting for process to die, giving up")
...
def remove_dag_run_tmp_data(dag_run):
    ...
    logging.info(
        f"Searching tmp data for dag_id: {dag_run.dag_id}, run_id: {dag_run.run_id}")
    tmp_folder_set = set()
    for ti in dag_run.get_task_instances():
        logging.info(
            f"Task: {ti.task_id}, execution_date: {ti.execution_date}, pid: {ti.pid}, state: {ti.state}")
        try:
            ...
            logging.debug(
                " - XCom was empty, or report file has been already deleted, or it's missing tmp_folder field")
...
def clean_dag_run_db(dag_run, session=None):
    ...
    logging.info(
        f"Cleaning DB for dag_id: {dag_run.dag_id}, run_id: {dag_run.run_id}")
    for ti in dag_run.get_task_instances():
        logging.info(
            f"Task: {ti.task_id}, execution_date: {ti.execution_date}, pid: {ti.pid}, state: {ti.state}")
...
def clean_up_dag_run(dag_id, run_id, dags_folder=None, kill_timeout=None):
    ...
    workflow. For that dag_id should follow the naming rule "wdlid-commitsha". If
    ...
    kill_timeout = 2 * \
        conf.getint(
            "core", "KILLED_TASK_CLEANUP_TIME") if kill_timeout is None else kill_timeout
...
def conf_get(
    ...
    try:
        ...
        # is guaranteed to be executed before any return
...
def get_default_wdl_args(preset_wdl_args=None):
    """
    Returns default arguments required by wdltool's functions with a few
    parameters added and overwritten (required by CWL-Airflow). Defaults
    can be preset through "preset_wdl_args" if provided. All new fields
    from "preset_wdl_args" will be added to the returned results.
    """

    preset_wdl_args = {} if preset_wdl_args is None else deepcopy(
        preset_wdl_args)

    # default arguments required by wdltool
    required_wdl_args = {}

    # update default arguments required by wdltool with those that were preset by user
    required_wdl_args.update(preset_wdl_args)

    # update default arguments required by wdltool with those that might
...
def relocate_outputs(
    ...
    "outputSource" if it was a list). "wdl_args" can be used to update
    default parameters used for loading and runtime contexts.
    """

    wdl_args = {} if wdl_args is None else wdl_args

    default_wdl_args = get_default_wdl_args(wdl_args)

    workflow_tool = fast_wdl_load(
        workflow=workflow,
        wdl_args=default_wdl_args
    )
    ...
    runtime_context = RuntimeContext(default_wdl_args)
    relocated_job_data = relocateOutputs(
        ...
        source_directories=[job_data_copy["tmp_folder"]],
...
def execute_workflow_step(
    ...
    and "task_id". "wdl_args" can be used to update default parameters
    ...
    default_wdl_args.update({                          # add execution specific parameters
    ...
    workflow_step_path = os.path.join(
        step_tmp_folder, task_id + "_step_workflow.wdl"
    )

    fast_wdl_step_load(                                # will save new worlflow to "workflow_step_path"
        workflow=workflow,
        target_id=task_id,
        wdl_args=default_wdl_args,
    ...
    workflow_data = slow_wdl_load(
        workflow=workflow_step_path,
        wdl_args=default_wdl_args
    )

    skipped = True
    step_outputs = {output_id: None for output_id,
                    _ in get_items(workflow_data.tool["outputs"])}
    if need_to_run(workflow_data, job_data, task_id):
        ...
        step_outputs, step_status = executor(
            workflow_data,
            job_data,
            RuntimeContext(default_wdl_args)
        )
        sys.stderr = _stderr
        ...
        # To remove "http://commonwl.org/wdltool#generation": 0 (copied from wdltool)
...
def load_job(
    ...
    inputs; never fails). "wdl_args" can be used to update parameters for
    ...
    "cwd". If "cwd" was None uses "inputs_folder" value from "wdl_args" or
    its default value returned from "get_default_wdl_args" function.
    ...
    default_wdl_args = get_default_wdl_args(wdl_args)
    cwd = default_wdl_args["inputs_folder"] if cwd is None else cwd

    loading_context = setup_loadingContext(
        LoadingContext(default_wdl_args),
        RuntimeContext(default_wdl_args),
        argparse.Namespace(**default_wdl_args)
    )

    job_copy = deepcopy(job)

    try:
        job_data, _ = loading_context.loader.resolve_ref(
            job_copy, checklinks=False)
    ...
    initialized_job_data = init_job_order(
        ...
        args=argparse.Namespace(**default_wdl_args),
        process=slow_wdl_load(
            workflow=workflow,
            wdl_args=default_wdl_args
        ),
...
def fast_wdl_step_load(workflow, target_id, wdl_args=None, location=None):
    """
    ...
    "wdl_args" can be used to update default location of "pickle_folder"
    used by "fast_wdl_load" as well as other parameters used by
    "slow_wdl_load" for loading and runtime contexts.
    ...
    for _, step_in in get_items(selected_step.get("in", [])):

        updated_sources = []  # to keep track of updated sources

        # "in" might not have "source"
        for step_in_source, _ in get_items(step_in.get("source", [])):
            ...
            try:
                ...
                upstream_step_tool = fast_wdl_load(
                    workflow=upstream_step["run"],
                    wdl_args=default_wdl_args
                )

                upstream_step_output = list(get_items(
                    {get_short_id(k, only_id=True): v for k, v in get_items(
                        upstream_step_tool["outputs"])},  # trick
    ...
    selected_step_tool = fast_wdl_load(
    ...
    for step_out, _ in get_items(selected_step["out"]):
        selected_step_output = list(get_items(
            {get_short_id(k, only_id=True): v for k, v in get_items(
                selected_step_tool["outputs"])},  # trick
        ...
        if "scatter" in selected_step:
            ...
            selected_step_output = deepcopy(selected_step_output)
...
def get_short_id(long_id, only_step_name=None, only_id=None):
    ...
    # if "id" has weird stuff between step name and id
    if len(part.split("/")) > 2:
        part = "/".join([part.split("/")[0], part.split("/")[-1]])

    # if fragment is only one word (based on "/")
    if len(part.split("/")) == 1:
...
def fast_wdl_load(workflow, wdl_args=None):
    """
    Tries to unpickle workflow from "pickle_folder" based on
    md5 sum of the "workflow" file. "wdl_args" can be used to update
    default location of "pickle_folder" as well as other parameters
    used by "slow_wdl_load" for loading and runtime contexts.

    If pickled file not found or failed to unpickle, load tool from
    the "workflow" using "slow_wdl_load" with "only_tool" set to True
    ...
    pickled_workflow = os.path.join(
        default_wdl_args["pickle_folder"],
...
def slow_wdl_load(workflow, wdl_args=None, only_tool=None):
    """
    Follows standard routine for loading WDL file the same way
    as miniwdl does it. "workflow" should be an absolute path
    the wdl file to load. "wdl_args" can be used to update
...
# replace load_tool with
    ...
    try:
        # guarantees that temp file will be removed
        with NamedTemporaryFile(mode="w") as temp_stream:
            json.dump(
                get_uncompressed(workflow, parse_as_yaml=True),
                temp_stream
            )
            # otherwise it might be only partially written
            temp_stream.flush()
...
def embed_all_runs(
    ...
    if it is Workflow. If not, doesn't replace anything. "wdl_args"
    ...
    if workflow_tool["class"] == "Workflow":
        ...
        __embed(workflow_tool_copy, wdl_args)
    else:
...
def convert_to_workflow(command_line_tool, location=None):
    ...
    if command_line_tool["class"] == "Workflow":
        ...
        workflow_tool = {
            "class": "Workflow",
            "wdlVersion": command_line_tool["wdlVersion"],
            "inputs": [],
            "outputs": []
        }
        ...
        for input_id, input_data in get_items(command_line_tool["inputs"]):
            workflow_input = {
                ...
                "type": remove_field_from_dict(input_data["type"], "inputBinding")
            }
